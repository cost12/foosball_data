...
class StatCollector:
    ...
    def strength(self, victory:bool, normalized:bool=False):
        ...
        for player in self.individual_stats['Name']:
            ...
            for opponent in self.matchup_stats[self.matchup_stats['Name']==player]['Opponent']:
                g = self.matchup_stats[(self.matchup_stats['Name']==player) & (self.matchup_stats['Opponent']==opponent)][consider].iloc[0]
                if normalized:
    ...
    def normed(self, wins:bool):
        ...
        for player in self.individual_stats['Name']:
            total = 0
            for opponent in self.matchup_stats[self.matchup_stats['Name']==player]['Opponent']:
                pct = self.matchup_stats[(self.matchup_stats['Name']==player) & (self.matchup_stats['Opponent']==opponent)]['W PCT'].iloc[0]
                if wins:
                    total += pct
                else:
                    total += 1-pct
    ...
    def get_times_scored_n(self, player1:str, player2:str, num:int) -> int:
        ...
        count = 0
        for game in self.filtered:
            if player1 in (game.winner, 'ANY') and player2 in (game.loser, 'ANY'):
    ...
    def get_stats(self, stat:str) -> pd.DataFrame:
        ...
        if stat in self.stats_from_players:
            ...
            print(f"Error: no stats for {stat}")
