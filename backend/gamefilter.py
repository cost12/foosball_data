...
class FilterType(Enum):
    RESTRICTED = auto() # select only games that have winner in winners AND loser in losers
    OPEN = auto()       # select only games that have winner in winners OR loser in losers
    SINGLE = auto()     # select only games that have player in winners OR player in losers where player plays as winner color
...
class GameFilter:
    ...
    def reset(self) -> None:
        ...
        self.winners = set()
        self.losers =  set()
        self.select_type = FilterType.RESTRICTED
    ...
    def get_predicate(self) -> Callable[[foosballgame.FoosballGame],bool]:
    ...
    def predicate(self, game:foosballgame.FoosballGame) -> bool:
        ...
        if not in_dates:
            return False
        if self.select_type == FilterType.RESTRICTED: # changes whether or or and
            if not (((game.winner in self.winners or "ANY" in self.winners) and \
                ...
                return False
        elif self.select_type == FilterType.OPEN:
