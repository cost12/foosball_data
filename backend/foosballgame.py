...
def get_records(games):
    records = {}
    for game in games:
        if not game.winner in records:
            records[game.winner] = [1,0]
        else:
            records[game.winner][0] += 1
        if not game.loser in records:
    ...
    for game in games:
        ...
        for m in matchups:
            if m.includes(game):
                ...
                break
        if not added:
            m2 = Matchup(game.winner,game.loser)
            m2.add_game(game)
            matchups.append(m2)
    return matchups
...
def get_normalized_record(games, p1):
    ...
    wins = 0
    losses = 0
    for record in opponent_records.values():
        wins +=   record[0]/(record[0]+record[1])
        losses += record[1]/(record[0]+record[1])
    return wins, losses
...
def get_streaks(games):
    substreaks = {}
    for game in games:
        if not game.winner in substreaks:
        ...
        if not game.loser in substreaks:
...
def get_average_scores(games):
    ...
    for player, player_totals in totals.items():
        avgs[player] = [div(player_totals[0], player_totals[1]), div(player_totals[2], player_totals[3])]
    return avgs
...
def goal_probs(games):
    ...
    probs = {}
    for player, player_goals in goals.items():
        probs[player] = (player_goals[0]+1)/float(player_goals[0]+player_goals[1]+2)
    return probs
...
def get_win_probabilities(games, p1score=0,p2score=0,gameto=10):
    ...
    win_probs = {}
    for player, player_gpbs in gpbs.items():
        win_probs[player] = get_win_prob(player_gpbs, p1score, p2score, gameto)
    return win_probs
